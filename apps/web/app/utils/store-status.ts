...
function getFutureOpening(store: StoreInfo, now: Date, currentDay: number) {
  for (let offset = 1; offset <= 14; offset += 1) {
    ...
    if (hours?.open) {
      const dayLabel = offset === 1 ? 'amanha' : WEEKDAY_LABEL_PT[currentDay === day ? day : day];
...
export function getStoreStatus(store: StoreInfo, date = new Date()): StoreStatus {
  ...
  return {
    isOpen: false,
    text: 'Fechado agora',
    nextText: getFutureOpening(store, date, clock.day),
  };
}
